...
Write-Host "=== StarterPack full scan ==="
Invoke-RunCode @'
local lines = {}
local function scan(obj, depth)
    ...
    if obj:IsA("Tool") then
        extra = string.format(" | TextureId='%s' Name='%s' ToolTip='%s' RequiresHandle=%s",
            obj.TextureId, obj.Name, obj.ToolTip, tostring(obj.RequiresHandle))
    end
