...
Write-Host "=== Set Tool icon, name, tooltip ==="
Invoke-RunCode @'
local tool = game:GetService("StarterPack"):FindFirstChildOfClass("Tool")
if not tool then return "Tool not found" end

local meshId = "124639498172051"

tool.TextureId = "rbxthumb:type=Asset&id=" .. meshId .. "&w=420&h=420"
tool.ToolTip   = "消火器"
tool.Name      = "消火器"

return string.format("OK: Name='%s' ToolTip='%s' TextureId='%s'",
    tool.Name, tool.ToolTip, tool.TextureId)
'@
